...
class GradualStyleBlock(Module):
    def __init__(self, in_c, out_c, spatial):
        ...
        modules = []
        modules += [Conv2d(in_c, out_c//2, kernel_size=1, stride=1, padding=0),
                    Conv2d(out_c//2, out_c//2, kernel_size=3, stride=2, padding=1, groups=out_c//2),
                    nn.LeakyReLU(inplace=True),
                    nn.InstanceNorm2d(out_c // 2),
                    Conv2d(out_c // 2, out_c, kernel_size=1, stride=1, padding=0),
                    nn.LeakyReLU(inplace=True)
                    ]
        for i in range(num_pools - 1):
            modules += [
                Conv2d(out_c, out_c//2, kernel_size=1, stride=1, padding=0),
                Conv2d(out_c//2, out_c//2, kernel_size=3, stride=2, padding=1, groups=out_c//2),
                nn.LeakyReLU(inplace=True),
                nn.InstanceNorm2d(out_c//2),
                Conv2d(out_c // 2, out_c, kernel_size=1, stride=1, padding=0),
