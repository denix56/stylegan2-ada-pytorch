...
def training_loop(
    ...
    if (augment_kwargs is not None) and (augment_p > 0 or ada_target is not None):
        augment_pipe = dnnlib.util.construct_class_by_name(**augment_kwargs) # subclass of torch.nn.Module
    ...
    net = StyleGAN2(G=G, D=D, G_opt_kwargs=G_opt_kwargs, D_opt_kwargs=D_opt_kwargs, augment_pipe=augment_pipe,
                    training_set=training_set, dataset_name=training_set_kwargs.name, batch_size=batch_gpu,
                    style_mixing_prob=loss_kwargs.style_mixing_prob, r1_gamma=loss_kwargs.r1_gamma,
                    pl_batch_shrink=loss_kwargs.pl_batch_shrink, pl_decay=loss_kwargs.pl_decay,
                    pl_weight=loss_kwargs.pl_weight, G_reg_interval=G_reg_interval, D_reg_interval=D_reg_interval,
                    ema_kimg=ema_kimg, ema_rampup=ema_rampup, metrics=[fid50k])

    trainer = pl.Trainer(gpus=num_gpus, accelerator='ddp', fast_dev_run=True,
...
#     if rank == 0:
#         print('Loading training set...')
#     #training_set = dnnlib.util.construct_class_by_name(** ) # subclass of training.dataset.Dataset
...
#         print(f'Distributing across {num_gpus} GPUs...')
#     ddp_modules = dict()
#     for name, module in [('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('E', E)]:
#         if (num_gpus > 1) and (module is not None) and len(list(module.parameters())) != 0:
#             module.requires_grad_(True)
#             module = torch.nn.parallel.DistributedDataParallel(module, device_ids=[device], broadcast_buffers=False)
#             module.requires_grad_(False)
...
#                 real_cs = torch.from_numpy(labels).to(device).split(batch_gpu)
#                 images = []
#                 for real_img, real_c in zip(real_imgs, real_cs):
#                     codes = E(real_img, real_c)
#                     images.append(G(codes, c=real_c, noise_mode='none').cpu())
